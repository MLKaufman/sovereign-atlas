...
SELECT
    ...
    (SELECT list(ga.alias ORDER BY ga.alias)
     FROM gene_aliases ga
     WHERE ga.gene_id = g.gene_id) AS gene_aliases,
    g.stable_id,
    ...
    CASE
        WHEN ct.parent_cell_type_id IS NULL
          OR ct.name IN ('B cell', 'T cell', 'macrophage') THEN NULL
        ELSE ct.name
    END AS cell_subtype,
