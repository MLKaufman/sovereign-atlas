...
def upsert_species(
    ...
    con.execute(
        ...
        ON CONFLICT(scientific_name) DO UPDATE SET
          ...
          taxonomy_id = coalesce(excluded.taxonomy_id, species.taxonomy_id)""",
        [scientific_name.strip(), common_name or None, taxonomy_id],
    )
    return con.execute(
        "SELECT species_id FROM species WHERE scientific_name = ?", [scientific_name.strip()]
    ).fetchone()[0]
...
def upsert_gene(
    ...
    con.execute(
        ...
        ON CONFLICT(species_id, symbol) DO UPDATE SET stable_id = coalesce(excluded.stable_id, genes.stable_id)""",
...
def upsert_cell_type(
    ...
    con.execute(
        ...
        ON CONFLICT(name, ontology_id) DO UPDATE SET description = coalesce(excluded.description, cell_types.description)""",
    ...
    *,
    ...
    con.execute(
        """INSERT INTO marker_assertions(
          ...
          developmental_stage, assay, confidence, human_verified, notes
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(gene_id, cell_type_id, marker_direction, tissue, condition, developmental_stage, assay)
        DO UPDATE SET confidence = excluded.confidence,
          human_verified = excluded.human_verified, notes = excluded.notes,
          updated_at = now()""",
        values,
    )
    return con.execute(
